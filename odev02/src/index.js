 ...
 * @param imgId
 */
export const showCard = (imgId) =>{
    const index = Math.floor(Math.random()*imgs.length) // images listesinde rastgele resim indexi seçer.

    document.getElementById(imgId).src = imgs[index].src; // Seçilen resim uzantısını gönderilen resim id sinin uzantısı ile değiştirir.
    //document.getElementById(imgId).onclick = null;  // Resimin tıklanılabilirligini kaldırır.

    if (imgs[index].class === "cat" && imgs.length  > 1){
    ...
    imgs.splice(index,1); // Seçilen resmin tekrar seçilmemesi için resmi images listesinden çıkarır.

    if(imgs.length === 1  && !found){
